/* SHA-256 (FIPS 180-4), dependency-free. Used for ELF/state binding. */
...
static void block(mb_sha256 *c, const uint8_t *p) {
...
void mb_sha256_final(mb_sha256 *c, uint8_t out[32]) {
	...
	uint8_t lenb[8];
	int i;
	mb_sha256_update(c, &pad, 1);
	while (c->fill != 56) { uint8_t z = 0; mb_sha256_update(c, &z, 1); c->len--; }
	for (i = 0; i < 8; i++) lenb[i] = (uint8_t)(bits >> (56 - 8*i));
	c->len -= 8;
	mb_sha256_update(c, lenb, 8);
	for (i = 0; i < 8; i++) {
