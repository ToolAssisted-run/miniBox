...
static void test_ro_read(void) {
	...
	long fd = mb_fs_open(fs, "f", O_RDONLY);
...
static void test_seek(void) {
	...
	CHECK_EQ(mb_fs_seek(fs, fd, -4, SEEK_END), (long)(sizeof(DOG)-1-4));
	CHECK_EQ(mb_fs_read(fs, fd, buf, 4), 4);
...
static void test_rw_write_grow(void) {
	...
	long fd = mb_fs_open(fs, "z", O_RDWR);
...
static void test_fd_semantics(void) {
	...
	long fa = mb_fs_open(fs, "a", O_RDONLY);
	long fb = mb_fs_open(fs, "b", O_RDONLY);
	CHECK_EQ(fa, 3);
	CHECK_EQ(fb, 4);
...
static void test_mount_errors(void) {
	...
	long fd = mb_fs_open(fs, "w", O_RDWR);
